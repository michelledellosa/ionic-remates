...
export class CouponProvider {
  ...
  checkNoItemInCartService = function (lineItems, coupon) {
    ...
    if (pCategory.length != 0) {
      for (let w of lineItems) {
        for (let x of w.categories) {
          for (let y of pCategory) {
